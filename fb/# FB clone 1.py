...
# Profile information
profile_name = "Batman"
profile_picture = ImageTk.PhotoImage(Image.open("facebook.png"))
...
friends_icon_image = ImageTk.PhotoImage(Image.open("Friends-Icon.png"))
...
img7 = Image.open("Earth_icon.png")
...
img7.putalpha(mask)
img7.resize((100,100))
earth_image = ImageTk.PhotoImage(img7)
...
earth_photo_label = Label(profile_frame, image=earth_image, bg="#3b5998")
earth_photo_label.pack(side=RIGHT, padx=5, pady=5)

friends_photo_label = Label(profile_frame, image=friends_icon_image, bg="#3b5998")
friends_photo_label.pack(side=RIGHT, padx=5, pady=5)

search_bar_image = ImageTk.PhotoImage(Image.open("search.png"))
search_bar_label = Label(profile_frame, image=search_bar_image, bg="#3b5998")
search_bar_label.pack(side=LEFT)

send_comment_image = ImageTk.PhotoImage(Image.open("send-comment-button.png"))
...
def is_toxic(comment):
    ...
    df1 = pd.read_csv('train.csv')  #english dataset
    df2 = pd.read_csv('hindi.csv')  #hindi dataset
    df=pd.concat([df1,df2])         #combined dataset

    df3 = pd.read_csv('negatives.csv')  #negative words
    negative = df3['negatives']
    negative = np.array(negative)   #array of negative words
    ...
    clf = MultinomialNB()       #NaiveBayes model
    clf.fit(X_train_vec, y_train)   #training model

    y_pred = clf.predict(X_test_vec)    #Prediction on test dataset

    accuracy = accuracy_score(y_test, y_pred)   #Accuracy
    print("Accuracy:", accuracy)

    # predict user input
    user_input_vec = vectorizer.transform([comment])
    is_toxic = clf.predict(user_input_vec)
    ...
    for word in com:    #counting negatives
        if word in negative:
            neg_count+=1

    #checking if toxic or not
    if is_toxic[0] == 1:  
        if neg_count%2==0:
            return True
        else:
...
def open_comment_window():
    ...
    comments_text = Text(comment_window, width=130, height=25)
    comments_text.pack()
    ...
    new_comment_entry = Entry(comment_window, width=100)
    new_comment_entry.pack()

    # create a button to submit comments
    submit_button = Button(comment_window, image=send_comment_image, command=lambda: submit_comment(comment_window, comments_text, new_comment_entry))
    submit_button.pack()
...
def submit_comment(comment_window, comments_text, new_comment_entry):
    ...
    if new_comment.strip():
        ...
        is_toxic_flag = is_toxic(new_comment)

        # display a message if the new comment is a palindrome
        if is_toxic_flag:
            messagebox.showerror("Error", "The entered comment is Toxic!")
...
postgroup = []
frame1 = Frame(posts_frame_canvas)
frame2 = Frame(posts_frame_canvas)
frame3 = Frame(posts_frame_canvas)
postgroup.append([frame1,frame2,frame3])
